...
// The AI is prompted (ai.ts) to always return four marker-delimited sections in
// a fixed order, so a tailor run's raw tailored_output can be parsed into its
// parts reliably rather than by hunting for loose headings. The markers are
// distinctive enough not to collide with real resume text.
//
// ===TAILORED_RESUME===
...
export function parseTailoredResume(output: string): TailoredSections {
  ...
  if (!sawMarker) {
    // Old-format row: no structured markers to work with.
